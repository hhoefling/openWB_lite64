...
log('mqtt/off.py Device:%d ip:%s neuer ueberschuss:%d' % (int(devicenumber), ipadr, uberschuss ))
...
publishOne("openWB/SmartHome/set/Devices/"+str(devicenumber)+"/ReqRelay", "0")
client.loop(timeout=0.2)
publishOne("openWB/SmartHome/set/Devices/"+str(devicenumber) +"/Ueberschuss", uberschuss)
client.loop(timeout=0.2)
client.disconnect()

file_stringpv = '/var/www/html/openWB/ramdisk/sm/device_' + str(devicenumber) + '_pv'
log('write 0 to %s' % (file_stringpv) )
