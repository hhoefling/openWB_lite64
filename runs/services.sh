...
#################################################################
function mqtt2mhi_cron5() # $1=eneabled
{
 # if enabed  start if not running
 # if disabled kill if running
 ...
 if (( $1 == 1  )) ; then
    ...
    if (( ${isrun:-0} == 0 )) ; then
      ...
      services_log "mqtt2mhi allready run"
    fi
 else
    services_log "mqtt2mhi disabled"
    if (( ${isrun:-0} != 0 )) ; then
       mqtt2mhi_stop
    else
      services_log "mqtt2mhi disabled and not running"
    fi
 fi 
}
function mqtt2mhi_reboot() # $1=eneabled
{
 # kill if running
 # start if enabled
 isrun=$(pgrep -f '^python.*/mqtt2mhi.py')
 if (( ${isrun:-0} != 0 )) ; then
    mqtt2mhi_stop
 else
   services_log "mqtt2mhi not running"
 fi
 if (( $1 == 1 )) ; then
    isrun=$(pgrep -f '^python.*/mqtt2mhi.py')
    services_log "mqtt2mhi enabled"
    ...
    services_log "mqtt2mhi disabled, not start needed"
 fi 
}
function mqtt2mhi_start()
{
...
function mqtt2mhi_status() # $1=eneabled
{
 if (( $1 == 1  )) ; then
    if pgrep -f '^python.*/mqtt2mhi.py' > /dev/null ; then
