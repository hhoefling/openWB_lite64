...
class Connection:
  ...
  # Init connection class
    def __init__(self, session, username, password, fulldebug=False, **optional):
        ...
        _LOGGER.info('Init Skoda Connect library, version{}'.format(lib_version))
        _LOGGER.debug('Using service {}'.format(self._session_base))
  ...
  # API login/logout/authorization
    ...
    async def _authorize(self, client='connect'):
        ...
        try:
            ...
            _LOGGER.debug('Starting authorization process for client {}'.format(client))
            ...
            if self._session_fulldebug:
                _LOGGER.debug('Get authorization page: "{}"'.format(authorizationEndpoint))
            try:
                req = await self._session.get(
    ...
    async def setDeparturetimer(self, vin, data, spin):
        ...
        try:
            ...
            # Modify timers if action is on, off or schedule
                ...
                setting['chargeMinLimit'] = int(data.get('schedule',{}).get('chargeMinLimit', 30)) ##Geht nicht
            ...
            print(f'POSTing the following timer data: {json.dumps(body)}')
            await self.set_token('vwg')
