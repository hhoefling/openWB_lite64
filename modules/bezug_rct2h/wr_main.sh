...
if (( debug > 2 )) ; then
  python3 $SELF/rct2.py -d --ip=$bezug1_ip  -w=$pvwattmodul >>/var/log/openWB.log 2>>$OPENWBBASEDIR/ramdisk/dbg.log 
else
 if (( debug > 1 )) ; then
   python3 $SELF/rct2.py --info --ip=$bezug1_ip  -w=$pvwattmodul >>/var/log/openWB.log 2>>$OPENWBBASEDIR/ramdisk/dbg.log
 else 
